describe Cline::Data, '#secrets' do
  ...
  it 'ignores extra unknown parameters from secrets file' do
    with_data(
      ...
      expect(secrets.cline_api_key).to eq 'sk_abf'
  ...
  it 'loads all secrets attributes' do
    with_data(
      ...
      secrets = data.secrets
      expect(secrets.cline_api_key).to eq 'sk_abcde'
      expect(secrets.open_ai_api_key).to eq 'sk_opena'
      expect(secrets.gemini_api_key).to eq 'sk_gemin'
      expect(secrets.api_key).to eq 'sk_apike'
      expect(secrets.aws_access_key).to eq 'sk_awsac'
      expect(secrets.aws_secret_key).to eq 'sk_awsse'
      expect(secrets.aws_session_token).to eq 'sk_awsto'
      expect(secrets.deep_seek_api_key).to eq 'sk_deeps'
      expect(secrets.open_ai_native_api_key).to eq 'sk_opnai'
      expect(secrets.open_router_api_key).to eq 'sk_oprtr'
      expect(secrets.lite_llm_api_key).to eq 'sk_litll'
      expect(secrets.sap_ai_core_client_id).to eq 'sk_sapcc'
      expect(secrets.sap_ai_core_client_secret).to eq 'sk_sapcs'
      expect(secrets.mistral_api_key).to eq 'sk_mistr'
      expect(secrets.zai_api_key).to eq 'sk_zai8c'
      expect(secrets.groq_api_key).to eq 'sk_groq8'
      expect(secrets.cerebras_api_key).to eq 'sk_cereb'
      expect(secrets.vercel_ai_gateway_api_key).to eq 'sk_vrcel'
      expect(secrets.baseten_api_key).to eq 'sk_baset'
      expect(secrets.requesty_api_key).to eq 'sk_rqsty'
      expect(secrets.fireworks_api_key).to eq 'sk_fwork'
      expect(secrets.together_api_key).to eq 'sk_togth'
      expect(secrets.qwen_api_key).to eq 'sk_qwen8'
      expect(secrets.doubao_api_key).to eq 'sk_douba'
      expect(secrets.moonshot_api_key).to eq 'sk_moons'
      expect(secrets.hugging_face_api_key).to eq 'sk_huggi'
      expect(secrets.nebius_api_key).to eq 'sk_nebiu'
      expect(secrets.asksage_api_key).to eq 'sk_askag'
      expect(secrets.xai_api_key).to eq 'sk_xai8c'
      expect(secrets.sambanova_api_key).to eq 'sk_samba'
      expect(secrets.huawei_cloud_maas_api_key).to eq 'sk_hwclm'
      expect(secrets.dify_api_key).to eq 'sk_dify8'
      expect(secrets.minimax_api_key).to eq 'sk_minim'
      expect(secrets.hicap_api_key).to eq 'sk_hicap'
      expect(secrets.aihubmix_api_key).to eq 'sk_ahubi'
      expect(secrets.nous_research_api_key).to eq 'sk_nousr'
      expect(secrets.wandb_api_key).to eq 'sk_wandb'
    end
  end
